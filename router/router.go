...
func RegisterRoutes(server *gin.Engine) {
	...
	r := server.Group("/api")
	{
		...
		v2 := r.Group("/v2")
		{
			v2.Group("/gorm-raw-sql").
				POST("/user", controllers.DemoRawInsertSql).
				GET("/user/first-detail", controllers.DemoRawSelecOnetSql).
				GET("/user", controllers.DemoRawSelectAllSql).
				DELETE("/user", controllers.DemoRawDeleteSql).
				PUT("/user", controllers.DemoRawUpdateSql)
		}
	}
}
